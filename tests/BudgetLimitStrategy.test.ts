...
describe('BudgetLimitStrategy (Feature 1)', () => {
  ...
  it('should group expenses by category correctly', async()=>{
    ...
    const testTransactions: Transaction[] = [
      { id: '1', date: '2026-05-01', amount: -150, category: 'Food', description: 'Grocery', status: 'completed' }, // Over budget
      { id: '2', date: '2026-05-02', amount: -900, category: 'Rent', description: 'Apartment', status: 'completed' }, // Under budget
    ];
    ...
    expect(result).toContain('* Rent: Budget = $1000, Spent = $900');
  ...
  it('should group expenses correctly by category and sum them', async()=>{
    ...
    const spy = vi.spyOn(BudgetService, 'getCategoryBudgets').mockResolvedValue(mockBudgets);
    const testTransactions: Transaction[] = [
      { id: '1', date: '2026-05-01', amount: -100, category: 'Food', description: 'Grocery', status: 'completed' }, 
      { id: '2', date: '2026-05-02', amount: -900, category: 'Food', description: 'Grocery', status: 'completed' }, 
    ];
    const result = await strategy.execute(testTransactions);

    expect(spy).toHaveBeenCalled();
    expect(result).toContain('Food');
    expect(result).toContain('Food: Budget = $100');
    expect(result).toContain('* Food: Budget = $100, Spent = $1000');
  ...
  it('should calculate absolute overage amounts and percentage exceeded', async()=>{
    ...
    const spy = vi.spyOn(BudgetService, 'getCategoryBudgets').mockResolvedValue(mockBudgets);
    const testTransactions: Transaction[] = [
      { id: '1', date: '2026-05-01', amount: -200, category: 'Food', description: 'Grocery', status: 'completed' }, 
      { id: '2', date: '2026-05-02', amount: -1500, category: 'Rent', description: 'Apartment', status: 'completed' }, 
    ];
    const result = await strategy.execute(testTransactions);

    expect(spy).toHaveBeenCalled();
    expect(result).toContain('* Food exceeded its budget by $100 (200.00% of budget)');
    expect(result).toContain('* Rent exceeded its budget by $500 (150.00% of budget)');
  });
  ...
  it('should handle scenarios where no categories are over budget',async()=>{
    ...
    const spy = vi.spyOn(BudgetService, 'getCategoryBudgets').mockResolvedValue(mockBudgets);
    const testTransactions: Transaction[] = [
      { id: '1', date: '2026-05-01', amount: -1, category: 'Food', description: 'Grocery', status: 'completed' }, 
      { id: '2', date: '2026-05-02', amount: -1, category: 'Rent', description: 'Apartment', status: 'completed' }, 
    ];
    const result = await strategy.execute(testTransactions);
  ...
  it('should handle empty transaction list gracefully', async() => {
    ...
    expect(spy).toHaveBeenCalled();
    expect(result).toContain('Not enough information given to generate report');
  });
});
